...
def main():
    ...
    if args.architecture.upper() == 'PREACTRESNET18':
        if args.dataset.upper() == 'TINYIMAGENET':
            model = PreActResNet18(kernel_size=7, stride=2, padding=3, num_classes=args.num_classes).cuda()
        else:
    ...
    if args.lr_schedule == 'cyclic':
        if args.dataset.upper() == 'SVHN':
            scheduler = torch.optim.lr_scheduler.CyclicLR(opt, base_lr=args.lr_min, max_lr=args.lr_max,
                                                          step_size_up=lr_steps * 2 / 5,
                                                          step_size_down=lr_steps * 3 / 5)
        else:
            scheduler = torch.optim.lr_scheduler.CyclicLR(opt, base_lr=args.lr_min, max_lr=args.lr_max,
